 ...
 *
 * use_delta_sync = 0 : dùng MPI_Allreduce(dist[], MAX) — O(n)/level (gốc)
 * use_delta_sync = 1 : dùng MPI_Allgatherv(new_vertices) — O(|frontier|)/level
 *
 * Tất cả MPI rank đều gọi hàm này với cùng giá trị use_delta_sync.
 * Kết quả dist[] chỉ hợp lệ ở rank 0. Gọi bfs_result_free() để giải phóng.
 */
