...
/* ------------------------------------------------------------------ *
 * NewVerts — buffer thu thập đỉnh mới trong 1 step (cho delta-sync)  *
 * Phải khai báo TRƯỚC top_down_step / bottom_up_step vì các hàm đó  *
 * nhận NewVerts* làm tham số.                                         *
 * ------------------------------------------------------------------ */
typedef struct {
    vertex_t *buf;
    int       count;
...
{
    (void)my_rank;  /* không dùng trực tiếp — MPI_Allgatherv tự biết rank */
    double t0 = MPI_Wtime();
...
BFSResult bfs_hybrid(const Graph *g, vertex_t source, int use_delta_sync)
{
    ...
    frontier_free(curr);
    frontier_free(next);
    if (use_delta_sync) new_verts_free(&local_new);
